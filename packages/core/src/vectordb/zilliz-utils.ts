...
export class ClusterManager {
  ...
  constructor(config?: ZillizConfig) {
    ...
    this.baseUrl =
      envManager.get("ZILLIZ_BASE_URL") ||
      config?.baseUrl ||
  ...
  async listProjects(): Promise<Project[]> {
    const response =
      await this.makeRequest<ListProjectsResponse>("/v2/projects");
  ...
  async describeCluster(clusterId: string): Promise<DescribeClusterResponse> {
    ...
    if (response.code !== 0) {
      throw new Error(
        `Failed to describe cluster: ${JSON.stringify(response)}`,
      );
    }
  ...
  async createFreeCluster(
    ...
    if (response.code !== 0) {
      throw new Error(
        `Failed to create free cluster: ${JSON.stringify(response)}`,
      );
    }
    ...
    while (Date.now() - startTime < timeoutMs) {
      try {
        ...
        if (clusterInfo.status === "RUNNING") {
          ...
          clusterInfo.status === "DELETED" ||
          clusterInfo.status === "ABNORMAL"
        ) {
          // Cluster creation failed
          throw new Error(
            `Cluster creation failed with status: ${clusterInfo.status}`,
          );
        }
    ...
    // Timeout reached
    throw new Error(
      `Timeout waiting for cluster ${clusterId} to be ready after ${timeoutMs}ms`,
    );
  }
  ...
  static async getAddressFromToken(token?: string): Promise<string> {
    ...
    try {
      ...
      const defaultProject = projects.find(
        (p) => p.projectName === "Default Project",
      );
      ...
      const clustersResponse = await clusterManager.listClusters(
      ...
      if (clustersResponse.clusters.length > 0) {
        ...
        console.log(
          `🎯 Using existing cluster: ${cluster.clusterName} (${cluster.clusterId})`,
        );
        ...
        console.log(
          `[ZillizUtils] ✅ Created new cluster: ${createResponse.clusterId}`,
        );
